...
input_directory = os.path.abspath(os.path.join(
    os.getcwd(), os.pardir, os.pardir, 'big_data_input', 'Dish'))
...
temp_directory = os.path.join(temp_directory, 'Dish')
if not os.path.isdir(temp_directory): os.mkdir(temp_directory)
...
output_directory = os.path.join(output_directory, 'Dish')
if not os.path.isdir(output_directory): os.mkdir(output_directory)

## Set input file name and acquisition parameters for processing
input_filename = ('transmitted_z_step_10 aluminium particle2.tif')
...
xslice = 122 # choose slice along x for display
