...
input_directory = os.path.abspath(os.path.join(
    os.getcwd(), os.pardir, os.pardir, 'big_data_input', 'Spirostomum'))
...
temp_directory = os.path.join(temp_directory, 'Spirostomum')
if not os.path.isdir(temp_directory): os.mkdir(temp_directory)
...
output_directory = os.path.join(output_directory, 'Spirostomum')
if not os.path.isdir(output_directory): os.mkdir(output_directory)

## Set input file name and acquisition parameters for processing
input_filename = (
    'Spirostomum - spx_cyan_100_vol_60um_stk_10um_step_10ms_exp3 worm-010_cropped.tif')
