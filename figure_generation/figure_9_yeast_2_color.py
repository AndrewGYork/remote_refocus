...
num_tps = 25 # Number of volumes or time points in series
...
for n in range(num_z_stack):
    ...
    for z in range(int(num_slices/2), num_slices - 1, 1):        
        ...
        plt.gca().get_xaxis().set_ticks([])
        plt.gca().get_yaxis().set_ticks([])
        plt.figtext(xmargin, ymargin + 2*space, '2 color volumetric time series',
...
copyfile(os.path.join(temp_directory, 'img000014.png'),
