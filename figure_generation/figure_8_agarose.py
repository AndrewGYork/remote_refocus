...
input_directory = os.path.abspath(os.path.join(
    os.getcwd(), os.pardir, os.pardir, 'big_data_input', 'Agarose'))
...
temp_directory = os.path.join(temp_directory, 'Agarose')
if not os.path.isdir(temp_directory): os.mkdir(temp_directory)
...
output_directory = os.path.join(output_directory, 'Agarose')
if not os.path.isdir(output_directory): os.mkdir(output_directory)

## Set input file name and acquisition parameters for processing
input_filename = ('transmitted_z_step_10_fresh_agarose_coiled_worm.tif')
...
xslice = 99 # choose slice along x for display
...
copyfile(os.path.join(temp_directory, 'img000150.png'),
